...
<b>Motivation:</b> In February 2020, I was unable to install ```npm``` using an R Shiny Electron template via Dirk Shumaker's <a href="https://github.com/dirkschumacher/r-shiny-electron">original repo</a> or Travis Hinkelman's <a href="https://github.com/hinkelman/r-shiny-electron">fork</a>. I bypassed this issue by installing elecron locally with ```npx``` instead, which unlike ```npm``` did not report dozens of warnings or vulnerabilities.
